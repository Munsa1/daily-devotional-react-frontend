...
const VerseOfTheDay = () => {
  ...
  // useEffect(() => {
  //   // Try to get devotional from localStorage (simulated backend)
  //   const savedDevotion = localStorage.getItem("latestDevotion");

  //   if (savedDevotion) {
  //     const data = JSON.parse(savedDevotion);
  //     setVerse(data.verse);
  //     setVerseText(data.devotion);
  //   } else {
  //     // fallback — default message or API verse
  //     fetch("https://labs.bible.org/api/?passage=votd&type=json")
  //       .then((res) => res.json())
  //       .then((data) => {
  //         setVerse(`${data[0].bookname} ${data[0].chapter}:${data[0].verse}`);
